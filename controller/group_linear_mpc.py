...
# MPC class for Group-based representation with linear prediction (CVM)
    ...
    def evaluate_rollouts(self):
        ...
        for i in range(self.num_rollouts):
            ...
            # compute the safe cost > calculate the distance between the rollouts and predictions
            if self.has_ped:
                min_dists = self._rollout_dist(self.rollouts[i], self.future_positions)
                safe_cost = self._min_dist_cost_func(min_dists)
            else:
                safe_cost = 0

            ## compute the smoothness cost
            action_diffs = np.diff(self.rollouts_action[i], axis=0) # self.rollouts_action shape: self.mpc_horizon, 2
            smooth_cost = np.sum(np.square(action_diffs))
            
            ## compute the position cost
            follow_pos = self.follow_state[0, :2] # follow_state: np.array([follow_pos[0], follow_pos[1], follow_speed, follow_motion_angle])
            position_cost = np.min(np.linalg.norm(self.rollouts[i] - follow_pos, axis=1))
            
            ## compute the velocity cost. Current version only include the position diff, not the velocity diff. ##
            ...
            # for ones that are included in the cost function.
            self.rollout_costs[i] = self.w_safe * safe_cost + self.w_follow * cost_follow
